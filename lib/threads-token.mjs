...
export function getThreadsSetupStatus() {
  const config = getAppConfig()
  const store = readTokenStore()
  ...
  try {
    ensureTokenDir()
    fs.accessSync(path.dirname(TOKEN_PATH), fs.constants.W_OK)
  ...
  return {
    appIdConfigured: Boolean(process.env.THREADS_APP_ID),
    appSecretConfigured: Boolean(process.env.THREADS_APP_SECRET),
    redirectUri: process.env.THREADS_REDIRECT_URI ?? null,
    oauthReady: Boolean(config),
    connected: Boolean(store?.accessToken),
    tokenExpiresAt: store?.expiresAt ?? null,
    dataDir: process.env.THREADS_DATA_DIR ?? "./data",
    dataDirWritable,
    connectUrl: config ? "/api/threads/oauth/start" : null,
  }
}
