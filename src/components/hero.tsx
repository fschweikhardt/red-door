...
function HeroTitleLine({ children, offset = "0.04em" }: HeroTitleLineProps) {
  return (
    <span
      className="hero-title-line"
      style={
        {
          "--hero-offset-x": offset,
          "--hero-offset-y": offset,
        } as React.CSSProperties
      }
    >
      <span className="hero-title-outline" aria-hidden>
        {children}
      </span>
      <span className="hero-title-fill">{children}</span>
    </span>
  )
}
